...
const SITEMAP_HINT = 'Site sitemap available. For navigation context, use the webcmd-browser-sitemap skill; treat browser state as truth if it disagrees.';
...
async function resolveBrowserTargetInSession(page, targetPage, opts) {
    ...
    try {
        ...
        throw new Error(`Target tab ${candidate} could not be validated in the current browser session. ` +
            'The browser runtime session may have restarted; re-run "webcmd browser tab list" and choose a current target.', { cause: err });
    }
...
`);
    ...
    browser.command('bind')
        ...
        if ((page && rawIndex) || (!page && !rawIndex)) {
            throw new BrowserCommandError('Bind requires exactly one Cloak tab target: --page <page-id> or --index <n>.', 'invalid_request', 'Run `webcmd browser <session> tab list`, then retry with the page id or index to bind.');
        }
